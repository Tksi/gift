...
- [x] 1.1 エラーコードから日本語メッセージへの変換ユーティリティを作成する
...
- [x] 3.1 ルーム作成フォームコンポーネントを作成する
  - プレイヤー人数選択ボタン (2〜7 人) を配置
  - デフォルトで 3 人を選択状態
  - 送信ボタンでルームを作成
  - タッチ操作に適したボタンサイズ (最小 44x44px)
  - _Requirements: 1.3, 1.4, 7.2_

- [x] 3.2 セッション作成ページを実装する
...
- [x] 4.1 (P) 中央カードとポット表示コンポーネントを作成する
...
- [x] 4.2 (P) プレイヤー情報パネルコンポーネントを作成する
  - プレイヤー名、所持チップ数を表示
...
- [x] 6.1 (P) ルールヒント表示コンポーネントを作成する
  - ヒントテキストを UI に表示
  - emphasis が "warning" の場合は警告スタイルで強調
  - 表示/非表示のトグルボタンを配置
  - _Requirements: 5.2, 5.3, 5.4_

- [x] 6.2 ヒント取得ロジックをゲームページに実装する
...
- [x] 8.1 ゲームページの状態管理とライフサイクルを実装する
...
- [x] 8.2 ゲームページのレイアウトとコンポーネント統合を行う
